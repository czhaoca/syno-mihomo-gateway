...
_msg_en() {
  case "$1" in
    ...
    menu_status)      printf '%s' 'Status / diagnose' ;;
    menu_deploy_saved_hint) printf '%s' ' (settings already saved - option 2 reuses them)' ;;
    step_express)     printf '%s' 'Express deploy - review the detected settings' ;;
    express_iface)    printf '%s' '  interface:    %s' ;;
    ...
    express_edit_hint) printf '%s' 'answer No to edit each value step by step instead' ;;
    ask_express)      printf '%s' 'accept all of the above and deploy with these settings' ;;
    warn_secret_empty) printf '%s' 'the dashboard secret is empty - anyone on the LAN could control the proxy' ;;
    ask_gen_secret)   printf '%s' 'generate a random dashboard secret now' ;;
    ok_secret_generated) printf '%s' 'random dashboard secret generated and saved to .env (CONTROLLER_SECRET)' ;;
    warn_gen_secret_failed) printf '%s' 'could not generate a secret (/dev/urandom unavailable) - set CONTROLLER_SECRET in .env' ;;
    warn_secret_none) printf '%s' 'leaving the dashboard UNAUTHENTICATED by your choice - set CONTROLLER_SECRET in .env to change this' ;;
    verify_title)     printf '%s' 'verification:' ;;
    verify_ok)        printf '%s' '  [ ok ]      %s' ;;
    verify_failed)    printf '%s' '  [FAILED]    %s' ;;
    verify_warn)      printf '%s' '  [WARN]      %s' ;;
    verify_skip)      printf '%s' '  [ -- ]      %s' ;;
    verify_controller) printf '%s' 'controller API answers inside the container' ;;
    ...
    verify_groups)    printf '%s' 'country proxy groups ("<Country> Auto") have nodes' ;;
    ...
    st_partial)       printf '%s' 'status: PARTIAL - containers or network exist without a running gateway (see Status / diagnose)' ;;
    st_deployed)      printf '%s' 'status: running - gateway %s - dashboard http://%s:%s' ;;
    status_title)     printf '%s' 'Deployment status' ;;
    status_state)     printf '%s' 'state:      %s' ;;
    status_mihomo)    printf '%s' 'mihomo:     %s (gateway IP %s)' ;;
    status_ui)        printf '%s' 'dashboard:  container %s' ;;
    status_dashboard) printf '%s' 'web UI:     http://%s:%s' ;;
    status_tun)       printf '%s' 'TUN mode:   %s' ;;
    ask_run_doctor)   printf '%s' 'run the full diagnostics (scripts/doctor.sh) now' ;;
    ok_doctor_done)   printf '%s' 'diagnostics completed - everything structural looks healthy' ;;
    warn_doctor_rc)   printf '%s' 'diagnostics finished with findings (exit %s) - see the messages above' ;;
    ...
    scan_dns_rollback) printf '%s' 'DNS variant write did not land - the split-horizon pair keeps the shipped defaults' ;;
    scan_dns_partial) printf '%s' 'DNS variant write failed midway - check DNS_CN_NAMESERVER/DNS_FOREIGN_NAMESERVER in .env by hand' ;;
    backfill_backup)  printf '%s' 'pre-v2 .env detected - backup saved to %s' ;;
    backfill_backup_kept) printf '%s' 'pre-v2 .env detected - keeping the existing pre-repair backup at %s' ;;
    backfill_wrote)   printf '%s' 'backfilled %s=%s (the split-horizon pair is required)' ;;
    warn_backfill_noexample) printf '%s' 'cannot backfill the split-horizon pair: .env.example not found - the render will name the missing variable' ;;
    diag_backfill_backup) printf '%s' 'cannot back up .env to %s - the split-horizon backfill was NOT applied' ;;
    diag_backfill_backup_fix) printf '%s' 'free space / fix permissions on the data dir, then rerun the deploy' ;;
    diag_backfill_write) printf '%s' 'backfilling %s did not land in .env (pre-repair backup: %s)' ;;
    diag_backfill_write_fix) printf '%s' 'check the data-dir filesystem, then rerun the deploy; the pre-repair state is in the backup' ;;
    ...
    diag_pg_provider_fix) printf '%s' 'seed the provider from the host: sudo sh scripts/seed_provider.sh - see TROUBLESHOOTING (Provider has no nodes)' ;;
...
_msg_zh() {
  case "$1" in
    ...
    menu_status)      printf '%s' '状态 / 诊断' ;;
    menu_deploy_saved_hint) printf '%s' '（已有保存的设置——选项 2 可直接复用）' ;;
    step_express)     printf '%s' '快速部署——请核对检测到的设置' ;;
    express_iface)    printf '%s' '  网卡：        %s' ;;
    ...
    express_edit_hint) printf '%s' '回答“否”可逐项修改各个取值' ;;
    ask_express)      printf '%s' '接受以上全部设置并以此部署' ;;
    warn_secret_empty) printf '%s' '面板密钥为空——局域网上的任何人都能控制代理' ;;
    ask_gen_secret)   printf '%s' '现在生成一个随机面板密钥' ;;
    ok_secret_generated) printf '%s' '已生成随机面板密钥并保存到 .env（CONTROLLER_SECRET）' ;;
    warn_gen_secret_failed) printf '%s' '无法生成密钥（/dev/urandom 不可用）——请在 .env 中设置 CONTROLLER_SECRET' ;;
    warn_secret_none) printf '%s' '按你的选择保持面板不设密钥——如需更改请在 .env 中设置 CONTROLLER_SECRET' ;;
    verify_title)     printf '%s' '部署验证：' ;;
    verify_ok)        printf '%s' '  [通过]      %s' ;;
    verify_failed)    printf '%s' '  [失败]      %s' ;;
    verify_warn)      printf '%s' '  [警告]      %s' ;;
    verify_skip)      printf '%s' '  [跳过]      %s' ;;
    verify_controller) printf '%s' '容器内控制器 API 有响应' ;;
    verify_tun)       printf '%s' 'TUN 接口与 IP 转发已启用' ;;
    verify_tun_off)   printf '%s' 'TUN 已关闭（纯代理模式）' ;;
    verify_ui)        printf '%s' '面板容器运行中' ;;
    verify_groups)    printf '%s' '国家代理分组（"<国家> Auto"）有可用节点' ;;
    ...
    st_partial)       printf '%s' '状态：不完整——存在容器或网络但网关未运行（见“状态 / 诊断”）' ;;
    st_deployed)      printf '%s' '状态：运行中 - 网关 %s - 面板 http://%s:%s' ;;
    status_title)     printf '%s' '部署状态' ;;
    status_state)     printf '%s' '状态：      %s' ;;
    status_mihomo)    printf '%s' 'mihomo：    %s（网关 IP %s）' ;;
    status_ui)        printf '%s' '面板容器：  %s' ;;
    status_dashboard) printf '%s' '网页界面：  http://%s:%s' ;;
    status_tun)       printf '%s' 'TUN 模式：  %s' ;;
    ask_run_doctor)   printf '%s' '现在运行完整诊断（scripts/doctor.sh）' ;;
    ok_doctor_done)   printf '%s' '诊断完成——结构性检查全部健康' ;;
    warn_doctor_rc)   printf '%s' '诊断发现问题（退出码 %s）——请查看上面的输出' ;;
    ...
    scan_dns_rollback) printf '%s' 'DNS 变体未写入——分域解析对保持出厂默认值' ;;
    scan_dns_partial) printf '%s' 'DNS 变体写入中途失败——请手动检查 .env 中的 DNS_CN_NAMESERVER/DNS_FOREIGN_NAMESERVER' ;;
    backfill_backup)  printf '%s' '检测到 v2 之前的 .env——备份已保存到 %s' ;;
    backfill_backup_kept) printf '%s' '检测到 v2 之前的 .env——保留已有的修复前备份 %s' ;;
    backfill_wrote)   printf '%s' '已回填 %s=%s（分域解析对为必填项）' ;;
    warn_backfill_noexample) printf '%s' '无法回填分域解析对：找不到 .env.example——渲染时将点名缺失变量' ;;
    diag_backfill_backup) printf '%s' '无法将 .env 备份到 %s——分域解析回填未执行' ;;
    diag_backfill_backup_fix) printf '%s' '请释放空间/修复数据目录权限后重新部署' ;;
    diag_backfill_write) printf '%s' '回填 %s 未写入 .env（修复前备份：%s）' ;;
    diag_backfill_write_fix) printf '%s' '请检查数据目录文件系统后重新部署；修复前状态保存在备份中' ;;
    ...
    diag_pg_provider_fix) printf '%s' '在宿主机上做种子恢复：sudo sh scripts/seed_provider.sh——参见 TROUBLESHOOTING（机场节点全部消失）' ;;
