...
#   default-empty  - Priority Nodes (the routing default) has no real nodes -> bad
...
# every-byte encoder carries SPACES as %20, which is what makes the renamed
# spaced groups URL-safe):
...
ENC_PRI='%50%72%69%6f%72%69%74%79%20%4e%6f%64%65%73'
...
GROUPS_FULL='{"proxies":[{"name":"All Nodes","type":"URLTest","all":["n1","n2","n3"]},{"name":"Priority Nodes","type":"URLTest","all":["n2"]},{"name":"日本","type":"URLTest","all":["n2"]},{"name":"美国","type":"URLTest","all":["n2"]},{"name":"PROXY","type":"Selector","all":["Priority Nodes","All Nodes","日本","美国","DIRECT","REJECT"]},{"name":"STREAMING","type":"Selector","all":["PROXY","All Nodes","日本","美国","DIRECT"]},{"name":"GLOBAL","type":"Selector","all":["All Nodes","PROXY"]}]}'
GROUPS_PREEPIC='{"proxies":[{"name":"All Nodes","type":"URLTest","all":["n1","n2","n3"]},{"name":"PROXY","type":"Selector","all":["All Nodes","DIRECT","REJECT"]},{"name":"STREAMING","type":"Selector","all":["PROXY","All Nodes","DIRECT"]},{"name":"GLOBAL","type":"Selector","all":["All Nodes","PROXY"]}]}'
...
# 1) healthy: All Nodes + Priority Nodes + two country groups, real nodes -> ok
ST=$(new_state healthy "$GROUPS_FULL")
printf '%s' "$REAL3" > "$ST/proxies-$ENC_ALL.json"
printf '%s' "$REAL1" > "$ST/proxies-$ENC_PRI.json"
...
# 3) Priority Nodes empty -> default-empty bad; empty country groups ride the detail
ST=$(new_state dempty "$GROUPS_FULL")
printf '%s' "$REAL3" > "$ST/proxies-$ENC_ALL.json"
printf '%s' "$EMPTY_REJECT" > "$ST/proxies-$ENC_PRI.json"
printf '%s' "$REAL1" > "$ST/proxies-$ENC_JP.json"
printf '%s' "$EMPTY_REJECT" > "$ST/proxies-$ENC_US.json"
run_pg "$ST"
grep -q '^proxy_groups|default-empty|bad|.*Priority Nodes' "$OUT_F" \
...
# 5) pre-epic config (no filtered groups) with a healthy All Nodes -> plain ok
...
# 6) pre-epic config with an EMPTY All Nodes -> provider-empty (DEC-A extension)
