...
render_human() {
  ...
  # the URL %20-encoded (Routing Mode -> Routing%20Mode).
  if [ "$CHECK_EGRESS" -eq 1 ] && [ "$_rh_broken" -eq 0 ]; then
    _rh_url="http://127.0.0.1:${CONTROLLER_PORT:-9090}/proxies/Routing%20Mode/delay?timeout=5000&url=http://www.gstatic.com/generate_204"
    _rh_header=""
