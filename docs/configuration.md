...
(the renderer fails loudly if any is empty — no DNS is hardcoded in the repo). A `#All Nodes`
suffix on a server is mihomo's group-detour fragment syntax: that resolver is dialed
**through** the group literally named `All Nodes` in the shipped template (rename both
together; CI verifies every fragment names a real group). The detour rides `All Nodes` — the
url-test group — rather than the `PROXY` selector on purpose: it always picks a live node by
...
| `PRIORITY_INCLUDE_FILTER` | | **Region scope for the routing default (applied first):** only provider nodes matching this regex are eligible for the **`Priority Nodes`** url-test group. The shipped default scopes it to **Japan**, so latency-driven switching stops hopping the egress *country* (login-sensitive sites flag accounts whose location keeps shifting). Same regexp2 flavor as below; tune it to *your* airport's naming — a pool with city-only names (东京/大阪) needs those words added. Zero matches ⇒ **REJECT** (fail closed; the doctor flags it — see [troubleshooting](troubleshooting.md#doctor-reports-an-empty-filtered-group-proxy_groups-priority-nodes-rejects--a-country-group-has-no-nodes)). Unset renders the group without a `filter:` line (all-minus-exclude). Requires `PRIORITY_EXCLUDE_FILTER`. | `日本\|JP\d\|^JP` |
| `PRIORITY_EXCLUDE_FILTER` | | Renders the **`Priority Nodes`** url-test group — provider nodes matching this regex removed, *after* the include filter above — as **`PROXY`'s first member**, i.e. the routing default. The shipped default excludes Hong Kong: many sites block HK egress IPs, and an HK-heavy pool makes latency-based selection land on an HK node almost every time. `All Nodes` keeps the **full** pool (the split-horizon DNS detour rides it), one dashboard click away. Syntax: regexp2 (.NET flavor) — case-sensitive unless `(?i)`, **unanchored substring** match (`香港` matches `香港01`), `\|` alternates; a **backtick refuses to render** (an invalid pattern crashes mihomo at startup). A pattern matching *every* node makes `Priority Nodes` **REJECT** (fail closed — never a silent unproxied bypass; the doctor flags it — see [troubleshooting](troubleshooting.md#doctor-reports-an-empty-filtered-group-proxy_groups-priority-nodes-rejects--a-country-group-has-no-nodes)). Unset/empty renders without the group. This key **replaced** `AUTO_EXCLUDE_FILTER`: a leftover line with the old name refuses the render with a rename error — delete it. | `香港\|HK\|(?i)hong ?kong` |
| `COUNTRY_GROUPS` | | Per-country auto-selection: `NAME=regex;NAME=regex;…` generates one **url-test** group per entry — the dashboard shows `NAME` and auto-picks the fastest node matching `regex` (regexp2, same flavor as `PRIORITY_EXCLUDE_FILTER`; anchor short Latin codes as `US\d\|^US`). A regex spanning several countries **is** a multi-country group (e.g. `亚洲=日本\|新加坡`). Groups appear in **both** `PROXY` and `STREAMING`, after `Priority Nodes`/`All Nodes`, in spec order; no own health probe (zero extra traffic); a zero-match group **REJECT**s when selected (fail closed; the doctor flags it). `NAME` may be CJK and may contain interior spaces (`Japan Auto`) — leading/trailing whitespace refuses — and must not shadow `All Nodes`/`Priority Nodes`/`PROXY`/`STREAMING`/`DIRECT`/`REJECT`; backtick, empty parts, duplicates and malformed entries **refuse to render**. Tune the shipped example to *your* airport's node names. Unset/empty renders without any generated group (existing `.env` files unchanged). | `US Auto=美国\|US\d\|^US;Singapore Auto=…` (see `.env.example`) |
...
that defaults to `Priority Nodes`/`All Nodes` (the fastest node from your subscription) and also offers `DIRECT` /
...
(`All Nodes` picks nodes by latency, which is rarely an unlock node). Rules must target a
