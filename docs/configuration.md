...
| `DNS_FOREIGN_NAMESERVER` | ✅ | **Split-horizon pair (b), v2 foreign-by-default:** rendered as the **default** `nameserver` — every domain *not* matched by a policy entry (geosite-listed foreign **and** the unlisted long tail) resolves ONLY here — overseas, tunneled via `#auto`. Those names never reach a domestic operator; a dead tunnel fails **closed** (SERVFAIL) instead of silently leaking (there is no fallback dual-query — that channel was removed with the legacy profile). | `https://1.1.1.1/dns-query#auto,https://8.8.8.8/dns-query#auto` |
