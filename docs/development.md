...
# shellcheck (via Docker, same 12 targets as CI)
docker run --rm -v "$PWD:/mnt" -w /mnt koalaman/shellcheck-alpine:stable \
  shellcheck -x install.sh scripts/gateway.sh scripts/auto_update.sh scripts/install_scheduler.sh \
  scripts/setup_network.sh scripts/render_config.sh scripts/package.sh scripts/doctor.sh \
  scripts/state_diff.sh bootstrap.sh scripts/lib/container.sh scripts/lib/targets.sh
