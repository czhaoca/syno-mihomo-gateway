...
| `shellcheck` | `sh -n` parse-checks **every** `*.sh` in the repo, then `shellcheck -x` on 13 targets: `install.sh`, `install-pi.sh`, `gateway.sh`, `auto_update.sh`, `install_scheduler.sh`, `setup_network.sh`, `render_config.sh`, `package.sh`, `doctor.sh`, `state_diff.sh`, `bootstrap.sh`, `lib/container.sh`, `lib/targets.sh` (sourced libs followed in-context) |
...
# shellcheck (via Docker, same 13 targets as CI)
