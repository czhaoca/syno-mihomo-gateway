...
## 升级 v1.3.8 之前的安装：渲染时报 `DNS_CN_NAMESERVER must be set`

在分域解析对出现之前创建的 `.env` 可以通过安装器的各项预检，却会在渲染时被拒绝
（错误会点名缺失的变量）。DSM 的**重新部署**预检（**修改** → **应用更改**同样会执行
该预检）与 lite 部署流程会自动修复——两者执行同一套回填：先将现网 `.env` 备份为
`.env.pre-v2.bak`（重试时保留首份备份——修复前的原始快照绝不会被覆盖），再从随包
`.env.example` 的默认值回填缺失的
`DNS_CN_NAMESERVER`/`DNS_FOREIGN_NAMESERVER` 列表，逐项校验写入确已落盘（写入失败
会在渲染之前中止部署并点名备份文件），并逐行打印写入内容。自定义值绝不会被改动，
修复也因此保持最小化；与全新安装相同的一次性网络探测变体升级，在每个字段都缺失
**或**仍为随包示例默认值时适用——因此修复中途出错后的重试仍保有该升级资格。

各平台的修复方法：

- **Synology DSM：** 运行 `sudo sh ./install.sh` → **重新部署**（或**修改** →
  **应用更改**——预检相同），并查看打印出的“已回填……”行。
- **树莓派 lite / 通用 Linux lite：** 重新运行 lite 部署（`sudo sh ./install-pi.sh` /
  `sudo sh ./install-linux.sh`）——它会在渲染之前以同样的方式回填该解析对。日常诊断
  （`sh scripts/pi/lite_ctl.sh doctor`，也可从两个安装器的“状态”菜单进入）同样会点名
  这条路径：其 BROKEN `config does not render`（配置无法渲染）结论会指出缺失的分域
  解析对，以及可完成回填的 lite 重新部署。
...
## 通用 Linux：macvlan 不友好主机（云虚拟机 / VPC / Wi-Fi）

**症状：** `install-linux.sh` 警告"检测到虚拟化/云主机……macvlan 子接口在此类环境常无法
转发局域网流量"——或者 compose 模式部署一切正常，但局域网设备访问不到 `MIHOMO_IP`
（ping/ARP 无响应），常见于云虚拟机（AWS/GCP/阿里云等）或虚拟交换机之下。

**原因：** macvlan 让网关以第二个、未知的 MAC 地址出现在主机端口上。云 VPC 会过滤未知
源 MAC 的帧，部分虚拟交换机也会丢弃——容器健康运行，但它的帧永远到不了局域网。Wi-Fi
的失效方式相同（[Wi-Fi 上的 macvlan 被拒绝](#树莓派wi-fiwlan0上的-macvlan-被拒绝)）。

**处理：** 这类主机上 lite 模式就是推荐答案——重新运行安装器，选择**部署**（模式向导
只在这里运行；"重新部署"沿用已保存的 compose 形态），并改选 lite：网关直接绑定主机
自己的 IP，完全不涉及 macvlan。之后把搁浅的 compose 容器对拆掉
（`docker compose --env-file ../syno-mihomo-gateway-data/.env down`），并把客户端指向
主机 IP。守卫的检测是启发式的；在桥接组网的家用实验室虚拟机
（Proxmox/ESXi 且未过滤 MAC）上 macvlan 明明可用时，确认警告、保留 compose 即可。背景：
[安装 — 通用 Linux 与树莓派](installation-linux.md#macvlan-不友好主机云--虚拟机)。

## 通用 Linux：选哪个镜像来源（REGISTRY_MODE）？

**症状：** 在 `install-linux.sh` 的镜像向导里不确定选 `docker` 还是 `acr`——或者选完
之后拉取立刻失败。

**原因：** 两个来源面向不同的网络。`docker` 通过多架构清单从上游公共仓库
（Docker Hub / ghcr.io）拉取——无封锁网络上开箱即用，但在中国大陆被屏蔽。`acr` 从你的
私有阿里云 ACR 镜像仓库拉取——中国大陆的答案，但需要自建镜像同步流水线，且默认流水线
只复制 amd64。

**处理：** 中国大陆以外保持向导默认的 `docker`。中国大陆选 `acr` 并搭建镜像同步流水线
（[自动更新 › ACR 配置](auto-update.md#acr-配置)）；arm64 主机上还需先同步 arm64——
下一条目的守卫在任何 arm64 主机上都会触发，不只是树莓派。
...
[安装 — 通用 Linux 与树莓派](installation-linux.md#compose-模式)。
...
就无法命中：流媒体到不了 `Streaming Unlock` 组，设备拨的是自己解析器给的（常被 GFW 污染的）
...
**原因：** 流媒体服务会封锁大多数机房出口 IP。你在 `Exit Country` 里所选国家对应的
`<Country> Auto` 分组按**该国之内的延迟**选节点，而延迟最低的节点很少具备流媒体解锁
能力——于是 Netflix 走的隧道本身没问题，但出口被 Netflix 拒绝。这是节点属性、不是规则
故障：`GEOSITE,NETFLIX,Streaming Unlock` 规则（及其音频服务同款——Spotify、Tidal、
Deezer、SoundCloud）把这些站点确定性地路由进独立的 `Streaming Unlock` 选择器
（默认 `Routing Mode`，即开箱行为不变）。

**解决：** 打开 MetaCubeXD → Proxies → `Streaming Unlock`，固定一个机场标注支持流媒体/Netflix
的节点（常见命名 `NF`、`流媒体`、`解锁`……）——或固定一个 `<Country> Auto` 分组，一键
锁定区域——然后重新加载片子。只有流媒体流量会切换；
其余流量继续走 `Routing Mode`。若解锁节点上仍失败：某些设备（智能电视、Android 的
**私人 DNS**）会绕过网关 DNS、按裸 IP 直连——自 v1.3.10 起嗅探器能从 SNI 恢复这些
连接的域名、让它们照常进入 `Streaming Unlock`，但设备自己的解析器若返回污染答案仍可能出
...
**解决：** 恢复机场（续费/更换订阅——分域解析 v2 是唯一的 DNS 配置档，没有可切回的
传统模式）。注意 DNS 绕行走的是**隐藏的** `All Nodes` 锚点分组（完整节点池，只为 DNS
保留——MetaCubeXD 不显示它的卡片），因此在面板里把 `Routing Mode` 固定为 `DIRECT`——或
`Exit Country` 所选分组为空——都**不会**弄断解析；只有订阅真正无节点才会。另外，新域名
的**首次**解析会多一个隧道往返（约几百毫秒）；mihomo 的 DNS
缓存在内存里，每次重启后都是冷的。

## 机场节点全部消失（国外网站打不开、节点列表为空）

**现象：** 面板 Proxies 页只剩分组卡片——`Routing Mode` / `Streaming Unlock` / `Exit Country` /
你的 `<Country> Auto` 分组——里面**没有任何机场节点**（`All Nodes` url-test 仍然存在，但
在面板中隐藏）；国外网站
...
## doctor 报告国家分组为空（`proxy_groups`：default-empty / country-empty）

**现象：** `doctor` 显示 `ERROR the Exit Country selection '…' has NO nodes …`（结果
BROKEN，状态 `default-empty`）或 `WARN country group(s) match no provider node: …`
（DEGRADED，状态 `country-empty`）；在面板中选中被点名的分组时所有连接都被拒绝而不是
被转发。

**原因：** 某条 `COUNTRY_GROUPS` 正则匹配不到任何机场节点。`default-empty` 是严重的那个：
空掉的正是 **`Exit Country` 当前所选**的国家分组，默认线路流量因此被 REJECT。
`country-empty` 表示空的是*其他*国家分组——默认线路仍然工作，但选中那个国家会被拒绝。
无论哪种，都是正则与机场的节点命名不再吻合——机场会不打招呼地改名（香港01 → HK-01），
而只用城市名（东京/大阪）的节点池从一开始就匹配不上 `日本|JP\d|^JP`。
这些分组按设计**失败关闭**（`empty-fallback: REJECT`）：流量被拦截，绝不悄悄直连出口——
doctor 就是让你发现*原因*的地方。

**解决：** 对照面板 Proxies 页的实际节点名检查正则，修改 `.env` 中对应的
`COUNTRY_GROUPS` 条目（语法见[配置说明](configuration.md)），
然后**重新部署**渲染（`sudo sh ./install.sh`）。修复期间的应急做法：在面板的
`Exit Country` 选择器里改选另一个国家。若 doctor 报告的是 `provider-empty`——**所有**
url-test 分组皆空——那是机场节点全部消失（见上一节），先做种子恢复
（`sudo sh scripts/seed_provider.sh`），与正则无关。

## 分组模型升级后，面板选择被重置了一次

**现象：** 升级到精简分组模型后，MetaCubeXD 里的所有选择都回到了各分组的第一个成员——
你固定的流媒体节点和模式选择都不见了。

**原因：** mihomo 的 `cache.db` 按**分组名**保存面板选择。本次精简重命名了各选择器并移除
了旧的过滤默认分组，保存的选择因此不再对应任何分组，每个被改名/移除的分组都回落到其
第一个成员。这只会发生**一次**（设计如此）；新的选择照常持久保存。

**解决：** 在 MetaCubeXD 里重新固定（`Routing Mode`、`Streaming Unlock`、`Exit Country`，
若启用了强制代理段还包括 `Full-Tunnel Devices`）。
期间默认值是合理的：`Exit Country` 骑乘你 `COUNTRY_GROUPS` 的**第一**项——既有 `.env`
保持自己原有的条目顺序，所以你的默认国家就是*你自己*的第一项。若你手工编辑 `.env` 而
不是重跑安装器：删除渲染错误点名的已退役过滤开关行（见
[配置](configuration.md#dns注入到配置模板中)——“已移除的开关”），确保 `COUNTRY_GROUPS`
已设置，`DNS_*` 的值保持原样——它们无需任何修改。
...
正式生效。你最后一次修改产生的配置要么渲染失败（例如 `COUNTRY_GROUPS` 里有反引号、
格式错误或整项缺失，DNS 缺失，某条 `DNS_*` 条目绕行的 `#某分组` 已不再渲染，或残留
已退役过滤开关的行——错误会点名需要删除的行），要么配置测试失败（例如 `COUNTRY_GROUPS`
中的无效正则——不加防护会让 mihomo 启动即崩溃并反复重启）。网关不会因此断网，而是**继续运行
上一份有效配置**；你的改动只是尚未生效。
