...
就无法命中：流媒体到不了 `Streaming Sites` 组，设备拨的是自己解析器给的（常被 GFW 污染的）
...
**原因：** 流媒体服务会封锁大多数机房出口 IP。你在 `Country Pick` 里所选国家对应的
`<Country> Auto` 分组按**该国之内的延迟**选节点，而延迟最低的节点很少具备流媒体解锁
能力——于是 Netflix 走的隧道本身没问题，但出口被 Netflix 拒绝。这是节点属性、不是规则
故障：`GEOSITE,NETFLIX,Streaming Sites` 规则（及其音频服务同款——Spotify、Tidal、
Deezer、SoundCloud）把这些站点确定性地路由进独立的 `Streaming Sites` 选择器
（默认 `Proxy Mode`，即开箱行为不变）。

**解决：** 打开 MetaCubeXD → Proxies → `Streaming Sites`，固定一个机场标注支持流媒体/Netflix
的节点（常见命名 `NF`、`流媒体`、`解锁`……）——或固定一个 `<Country> Auto` 分组，一键
锁定区域——然后重新加载片子。只有流媒体流量会切换；
其余流量继续走 `Proxy Mode`。若解锁节点上仍失败：某些设备（智能电视、Android 的
**私人 DNS**）会绕过网关 DNS、按裸 IP 直连——自 v1.3.10 起嗅探器能从 SNI 恢复这些
连接的域名、让它们照常进入 `Streaming Sites`，但设备自己的解析器若返回污染答案仍可能出
...
保留——MetaCubeXD 不显示它的卡片），因此在面板里把 `Proxy Mode` 固定为 `DIRECT`——或
`Country Pick` 所选分组为空——都**不会**弄断解析；只有订阅真正无节点才会。另外，新域名
的**首次**解析会多一个隧道往返（约几百毫秒）；mihomo 的 DNS
缓存在内存里，每次重启后都是冷的。

## 机场节点全部消失（国外网站打不开、节点列表为空）

**现象：** 面板 Proxies 页只剩分组卡片——`Proxy Mode` / `Streaming Sites` / `Country Pick` /
...
**现象：** `doctor` 显示 `ERROR the Country Pick selection '…' has NO nodes …`（结果
...
空掉的正是 **`Country Pick` 当前所选**的国家分组，默认线路流量因此被 REJECT。
...
`Country Pick` 选择器里改选另一个国家。若 doctor 报告的是 `provider-empty`——**所有**
...
**解决：** 在 MetaCubeXD 里重新固定（`Proxy Mode`、`Streaming Sites`、`Country Pick`）。
期间默认值是合理的：`Country Pick` 骑乘你 `COUNTRY_GROUPS` 的**第一**项——既有 `.env`
