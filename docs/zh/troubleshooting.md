...
**原因：** 流媒体服务会封锁大多数机房出口 IP。`All Nodes` 组按**延迟**选节点，而延迟最低的
...
其余流量继续走 `PROXY`。若解锁节点上仍失败：某些设备（智能电视、Android 的
...
传统模式）。注意 DNS 绕行
走的是 `All Nodes` 组，在面板里把 `PROXY` 固定为 `DIRECT` **不会**弄断解析——只有订阅真正
无节点才会。另外，新域名的**首次**解析会多一个隧道往返（约几百毫秒）；mihomo 的 DNS
缓存在内存里，每次重启后都是冷的。

## 机场节点全部消失（国外网站打不开、节点列表为空）

**现象：** 面板 Proxies 页只剩 `All Nodes` / `DIRECT` / `REJECT`，没有任何机场节点；国外网站
...
## doctor 报告过滤分组为空（`proxy_groups`：Priority Nodes 拒绝流量 / 某国家分组没有节点）

**现象：** `doctor` 显示 `ERROR default route Priority Nodes has NO nodes …`（结果 BROKEN）或
`WARN country group(s) match no provider node: …`（DEGRADED）；在面板中选中该分组时所有
连接都被拒绝而不是被转发。

**原因：** 过滤正则匹配不到任何机场节点。`PRIORITY_INCLUDE_FILTER` / `PRIORITY_EXCLUDE_FILTER`
（默认线路 `Priority Nodes`）或某条
...
`PRIORITY_INCLUDE_FILTER` / `PRIORITY_EXCLUDE_FILTER` / 对应的 `COUNTRY_GROUPS` 条目
（语法见[配置说明](configuration.md)），
然后**重新部署**渲染（`sudo sh ./install.sh`）。修复期间的应急做法：在面板 PROXY 选择器里
选 `All Nodes`（未过滤的完整节点池）。若 doctor 报告的是 `provider-empty`——**所有** url-test
...
格式错误、DNS 缺失），要么配置测试失败（例如 `PRIORITY_INCLUDE_FILTER`/`PRIORITY_EXCLUDE_FILTER`/`COUNTRY_GROUPS` 中的
