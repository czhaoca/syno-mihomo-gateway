...
**诊断：** 当 NAS 的局域网接口上存在全局 IPv6 地址时，`doctor` 告警
`ipv6_bypass: exposed`——NAS 和客户端在同一个二层网段，它有全局地址即证明路由器的
通告能到达所有设备。在客户端上，`ipconfig` / `ip -6 addr` 里出现任何全局
（非 `fe80::`）地址，就说明这台设备拥有绕过通路。
