...
（错误会点名缺失的变量）。重新部署的预检会自动修复：先将现网 `.env` 备份为
`.env.pre-v2.bak`（重试时保留首份备份——修复前的原始快照绝不会被覆盖），再从随包
`.env.example` 的默认值回填缺失的 `DNS_CN_NAMESERVER`/`DNS_FOREIGN_NAMESERVER`
列表，逐项校验写入确已落盘（写入失败会在预检阶段中止部署并点名备份文件），并逐行
打印写入内容。已存在的值——包括自定义值——绝不会被改动；只有**两个**列表都缺失的
文件才会获得与全新安装相同的一次性网络探测变体升级。因此修复方法就是：运行
`sudo sh ./install.sh` → **重新部署**，并查看打印出的“已回填……”行。
