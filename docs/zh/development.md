...
scripts/
  ...
  lib/
    ...
    panel.sh                  # 网关面板 HTTP API 的 docker-exec 轻量客户端
...
sh scripts/package.sh --profile linux         # enduser 集合 + 通用 Linux 移植（-linux 产物；'pi' 为带警告的别名）
...
- 默认（最终用户）配置会通过 `package.sh` 中的 `ENDUSER_EXCLUDES` 路径规格剔除开发者/CI
  ...
  禁止字符串，发现即以失败告终（fail-closed）。linux profile 的 `LINUX_EXCLUDES` 由
  `ENDUSER_EXCLUDES` **派生**（仅减去六个通用移植路径）；禁止列表分为两组：身份字符串对两种
  精选 profile 都生效，代码托管域名仅对 enduser 生效。新增被跟踪文件或禁止字符串时，请保持
  `scripts/package.sh` 与 `scripts/ci/package_check.py` 同步（两处都携带该分组）。
...
5. 对通过验证的那个 SHA 打标签。
6. 携带八个发布包资产（每个包各自的 zip、tar.gz 及两个 `.sha256` 校验文件）发布
   GitHub Release。
7. 删除上一个 Release **及**其标签——仓库只保留最新一个版本。

> **改名后首个发布版本的 Release Notes 提示：**原先的 `syno-mihomo-gateway-pi-*` 资产现在
> 以 `syno-mihomo-gateway-linux-*` 名称发布（linux 包同时携带 `install-pi.sh` 与
> `install-linux.sh`）。在只保留最新版本的策略下，旧的 `-pi` 下载链接会消失——请在
> Release Notes 中明确说明这次改名。
...
# shellcheck（经 Docker，与 CI 相同的 23 个目标）
