...
（`DOCKER_REGISTRY/ACR_NAMESPACE/...`）。系统**不做上游→ACR 名称转换**：若容器运行的是
Docker Hub 引用，请先镜像该映像（加入 `docker-china-sync/images.txt`），用 ACR 引用重新
部署容器后再登记。

**登记 / 管理**（两者都只编辑 `<数据目录>/state/update-targets` 管理列表，绝不写 `.env`）：

```sh
sudo sh scripts/gateway.sh update --enable <容器名> --yes
sh scripts/gateway.sh update --list-targets     # 只读，显示实时可用性
sh scripts/gateway.sh update --last             # 只读，最近一次运行结果
sudo sh scripts/gateway.sh update --disable <容器名> --yes
```

也可通过安装器的更新流程交互式管理（扫描运行中的 ACR 容器逐个开关）。无论是否登记，
以下容器永不参与：网关三件套（由各自路径更新）、compose 管理或标签不完整的容器（绝不
手工重建）、匹配 `UPDATE_DENY_CONTAINERS` 通配的容器、`--rm`/`container:*` 模式的容器，
以及回放引擎无法完整复现其设置的容器（防降级守卫会拒绝并指出该设置）。疑似数据库的
镜像在登记时会发出明确警告：重建没有静默机制。

**通用目标的更新方式**（严格串行，先于 cloudflared，网关对永远最后）：从 `docker
inspect` 捕获完整运行规格（环境、挂载——匿名卷按名保留——端口、网络/静态 IP/别名、
capabilities、设备、资源限制、标签、日志配置……），用新镜像原地重建，并须通过分层健康
门：运行中 → 重启计数稳定 → 镜像自带 healthcheck（若有）→ 可选的每目标探针
（`exec:<命令>` 或 `log:<正则>`，在容器网络命名空间内执行）。失败时自动用保存的规格 +
旧镜像恢复；成功后将验证过的镜像 ID 持久化到 `<数据目录>/state/last-good/`（后续清理
不影响回滚），并在 `state/last-run.json` 记录本次运行供 `gateway.sh status` 展示。
...
2. **金丝雀** —— 登记一个非关键容器，然后：
   ...
   ```sh
   sudo sh scripts/state_diff.sh snapshot <容器名> /tmp/canary-before
   sudo sh scripts/gateway.sh update --yes
   sudo sh scripts/state_diff.sh compare <容器名> /tmp/canary-before
   ```

   `state_diff.sh` 使用更新器自身的捕获引擎做快照，"状态保留"因此是机械化的相等断言
   （镜像/身份字段豁免）。出现任何 `DRIFT:` 行即验收失败——仅有一个狭窄例外：
   `labels`/`hc-test`/`hc-meta` 是相对各自镜像的覆盖差集，若新镜像恰好内置了逐字节相同
   的覆盖值，可能出现误报；将这两类判为真实回归前请先用 `docker inspect` 手动比对。
