...
经网关面板管理动态设备策略（列出条目，或设置某设备的模式）—— 面板 HTTP API 之上的轻量令牌鉴权代理。

| 选项 | 说明 |
|---|---|
| `--list` | 打印面板的设备文档（JSON，机器可读的透传） |
| `--set ADDRESS` | 为一个 IPv4 地址或 CIDR 设置策略（需要 --mode；由面板做失败即停的 校验与规范化） |
| `--mode full-tunnel\|full-direct\|default` | 与 --set 连用：目标模式；'default' 表示移除该条目 |
| `--name NAME` | 与 --set 连用：设备显示名（可选） |
| `--note NOTE` | 与 --set 连用：记录到审计条目的备注（可选） |
...
status --json 输出一个扁平对象： {"verb","ok","exit_code","stack_state","mihomo_ip","dashboard_url", "checks":[{"name","value"},...],"last_update":{...}|null} （last_update 与 state/last-run.json 一致；首次运行前为 null）。 其检查名为 env、docker、mihomo_container、ui_container、network、 subscription、tun_enable。last-run.json 携带 {"ts","exit_code","dry_run","updated","unchanged","failed", "rolled_back","updated_names","failed_names","rolled_back_names"}。 doctor --json 输出 {"verb","ok","exit_code","checks":[...]}，检查名为 env、docker、arch、tun_device、network、compose、mihomo、tun_gateway、 controller、image_arch、proxy_groups、dashboard、update_task、boot_task、 cloudflared、 subscription、host_dns、geodata、dns_privacy、config_rejected、 ipv6_bypass、full_proxy（env 取值 ok|needs-migration|broken —— needs-migration 表示 .env 可解析但早于网关面板时代 （PANEL_IMAGE/PANEL_IP 未设置）；在 install.sh 迁移（只询问 PANEL_IP）完成之前，部署与更新保持拒绝； update_task/boot_task 校验 DSM 任务计划的部署情况；proxy_groups 针对生成的“<国家> Auto” url-test 分组， 取值 ok|default-empty|country-empty|provider-empty —— default-empty 表示 Exit Country 当前选中的国家分组匹配不到任何节点（默认线路流量被 REJECT，失败关闭）， country-empty 表示其他 COUNTRY_GROUPS 条目匹配不到节点， provider-empty 表示所有 url-test 分组皆空（机场节点未就绪——先做 种子恢复，不是过滤器的问题）；cloudflared 对可选的隧道容器报告 ok|down|absent；host_dns 逐一探测主机解析器；geodata 为 cached|missing；dns_privacy 反映渲染出的 DNS 配置档，取值 v2|v1|legacy —— v2 = 分域（境外优先）隐私模式，v1 = v1.3.10 之前的 渲染残留（fallback 双查询仍会把长尾查询抄送国内解析器），legacy = 国内解析器可见全部查询；config_rejected 取值 ok|render-failed|config-test-failed|rejected —— 非 ok 表示入口点守门 拒绝了最近一次渲染的配置（存在 .config.yaml.rejected 标记），mihomo 正运行上一份配置，最近的 .env/订阅修改并未生效（'rejected' = 标记存在但 reason 行不可读）；渲染通过并重新部署后自动清除； ipv6_bypass 取值 ok|exposed —— exposed 表示 LAN 接口上存在可路由互联网的全局 IPv6（私有 ULA 不算）， 双栈客户端会经 IPv6 绕过纯 IPv4 网关；full_proxy 针对整机代理网段（FULL_PROXY_SOURCES），取值 disabled|ok|parity-drift|chain-violation —— disabled 表示未启用网段， parity-drift 表示渲染出的 SRC-IP-CIDR 规则与 .env 不一致（网段修改 未生效——请重新部署），chain-violation 表示网段设备的非局域网流量 绕过了 Full-Tunnel Devices 分组（通常是已在文档说明的 UDP/QUIC 穿透：出口 节点不支持 UDP 中继；该保证同样要求局域网没有可路由 IPv6——参见 ipv6_bypass；自面板史诗起，该检查还会扫描来自 providers/dyn-full-tunnel.txt 的动态全隧道来源）； companion_health 针对网关面板伴随容器，取值 ok|degraded|down|absent ——absent（未部署）为静默，down/degraded 仅告警：路由不受影响 （provider 文件失败即静态保持），但策略编辑与统计采集不可用； policy_parity 针对面板自身的 数据库→provider 文件→mihomo 应用核对 （经面板 /health 读取），取值 ok|drift|unverified——drift 为 ERROR： 最近一次策略变更已保存但并未到达 mihomo（请在面板中重新应用；持久的 panel-apply-failed 标记与之对应）；面板不存在时不输出记录； "unknown" 表示此机器上该项不可探测）。 标准输出只有这一个对象，日志绝不混入。
