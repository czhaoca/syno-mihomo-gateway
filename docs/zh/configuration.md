...
| 键 | Req | 说明 | 示例 |
|---|:--:|---|---|
| `DNS_DEFAULT_NAMESERVER` | ✅ | 引导解析器（纯 IP，用于解析其他解析器）。 | `223.5.5.5,119.29.29.29` |
| `DNS_NAMESERVER` | ✅ | 主用/国内解析器。 | `223.5.5.5,119.29.29.29` |
| `DNS_FALLBACK` | ✅ | 境外/抗污染解析器；允许并推荐 DoH（`https://…`）/ DoT（`tls://…`）条目。 | `https://1.1.1.1/dns-query,tls://8.8.8.8:853` |

`.env.example` 的出厂默认值与 `REGISTRY_MODE=acr` 的中国大陆定位一致（bootstrap/国内列表
用纯 IP 国内解析器，抗污染 fallback 用加密解析器）；在无过滤的网络上三项都可用
`1.1.1.1,8.8.8.8`。这些设置只作用于**网关**——NAS 自己的解析器（DSM 控制面板 → 网络）
也必须可达，`doctor` 现在会逐一探测（`host_dns` 检查）。部署时还会通过 CDN 镜像预下载
geo 数据库（`GEODATA_MIRRORS` 可覆盖镜像列表），首次启动不再依赖跨境下载。
