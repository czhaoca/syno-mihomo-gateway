...
| 观察方 | 能看到什么 | 什么会改变它 |
|---|---|---|
| 国内解析服务运营方——AliDNS（阿里）、DNSPod（腾讯） | 中国列表内的域名（经加密 DoH），加上引导钉扎（geo 镜像、健康检查主机、机场面板、节点域名）。**仅此而已**——境外列表与长尾域名完全不会到达这里；从局域网客户端跑 dnsleaktest.com 扩展测试不会出现国内解析器。 | 分域解析对不可取消（留空会拒绝渲染）；已退役的传统配置档曾把**所有**域名送到这里——v1.3.10 之前的渲染也会经 `GEOIP,CN` 解析 + fallback 双查询泄漏长尾；`doctor` 以 `dns_privacy` 报告渲染出的配置档。 |
...
面板还从 fake-ip 中排除——无条件生效（分域解析 v2 是唯一的配置档，无开关）。
...
启、全程加密、境外与长尾路径走隧道）；在无过滤的网络上，可将所有列表都设为境外解析器
（如 `1.1.1.1,8.8.8.8`）；至少应将分域解析对——它绝不能留空（留空会拒绝渲染）——改为
去掉 `#All Nodes` 绕行后缀的境外服务器。全新安装的网络探测判定为无过滤时，安装器会自动
应用这一解析对变体（引导解析器列表保持出厂值；`SMG_SCAN_*` 变量可覆盖探测端点或强制
指定判定结果）。这些设置只作用于**网关**——NAS 自己的解析器（DSM
...
> 时才设为 `docker`。通用 Linux 安装器（`install-linux.sh`）会把**新**安装默认为 `docker` ——
> 上游镜像是多架构的，arm64 主机无需镜像同步流水线；无论如何，仓库提交的 `.env.example` 保持
> `acr` 默认值。
...
| `EXPECTED_ARCH` | 防止仅 amd64 的镜像源被装到 ARM 架构的机器上。可选 `amd64` / `arm64` / `arm`（32 位，如 armv7 树莓派）。树莓派与通用 Linux 安装器会自动固定为本机架构；只有 DSM 路径保持 `amd64` 默认值。 | `amd64` |
...
| `INSTALLER_ENTRY` | **自动管理的平台键** —— 通常无需手工设置。`install-linux.sh` / `install-pi.sh` 会把它设置并导出为各自的入口名，并尽力持久化进你的 `.env`（绝不创建该文件），使独立运行的脚本（`doctor.sh`、`gateway.sh`、`auto_update.sh`、cron）在修复提示中给出正确的安装器名字。未设置（DSM 路径从不设置它）= 保持 `install.sh` 措辞。特意不收录在 `.env.example` 中。 | *（未设置）* |
| `PLATFORM_LABEL` | `INSTALLER_ENTRY` 的**自动管理**姊妹键：`linux` / `pi` 让共享脚本的修复提示换用通用措辞；未设置 = DSM 措辞。设置与持久化方式同上；删掉它只会让独立运行的提示退回 DSM 措辞，下次运行安装器时会自动恢复。 | *（未设置）* |
