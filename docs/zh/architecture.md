...
| 平台 | Compose 模式（本页拓扑） | Lite 模式（无 Docker） |
|---|---|---|
| **群晖 DSM** | ✓ —— 权威部署 | — |
| **树莓派** | ✓（64 位系统，有线以太网） | ✓ |
| **通用 Linux（amd64/arm64）** | ✓（64 位系统、有线以太网、macvlan 可行的网络） | ✓ |

支持层级（权威矩阵，本页刻意不复述）与各平台完整流程见
...
同一套组件也能跑在**通用 Linux 主机或树莓派**上（`sudo sh ./install-linux.sh` /
`sudo sh ./install-pi.sh`——附加入口；上面的 DSM 路径不受影响），有两种形态：*compose 同构*
